...
// ── /admin/republish/:post_id ─────────────────────────────────────────
// One-shot retry: fetches an already-approved post from the DB and
// re-fires social publishing. Remove after use.
app.post('/admin/republish/:post_id', async (req, res) => {
  const { post_id } = req.params;
  if (!isServerAvailable('web')) {
    res.status(503).json({ success: false, error: 'Web MCP server unavailable' });
    return;
  ...
  try {
    // Fetch the approved post from the DB
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const raw = await callTool('web', 'web_get_post', { post_id }) as any;
    if (raw?.isError === true) {
      res.status(404).json({ success: false, error: raw?.content?.[0]?.text ?? 'Post not found' });
      return;
    }
    const text = raw?.content?.[0]?.text;
    const post = text ? JSON.parse(text) as Record<string, unknown> : null;
    if (!post) {
      res.status(404).json({ success: false, error: 'Could not parse post from DB' });
      return;
    }

    // Reconstruct InjuryPostContent — same logic as /admin/approve
    const rtpRaw: Record<string, unknown> = {
      min_weeks:          post.return_to_play_min_weeks,
      max_weeks:          post.return_to_play_max_weeks,
      probability_week_2: post.rtp_probability_week_2,
      probability_week_4: post.rtp_probability_week_4,
      probability_week_8: post.rtp_probability_week_8,
      confidence:         post.rtp_confidence,
    };
    ...
    const content: InjuryPostContent = {
      ...
      confidence: Number(post.rtp_confidence ?? 0),
      ...(post.conflict_reason ? { conflict_reason: String(post.conflict_reason) } : {}),
      ...(post.team_timeline_weeks !== undefined ? { team_timeline_weeks: Number(post.team_timeline_weeks) } : {}),
      ...(post.parent_post_id ? { parent_post_id: String(post.parent_post_id) } : {}),
    };
    ...
    console.log(`[Republish] Retrying social publish for post ${post_id} (${content.athlete_name})`);
    const result = await publishApprovedDeepDive(content, postUrl, post_id);
    res.json({ success: true, result });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[Republish] Failed for post ${post_id}: ${message}`);
    res.status(500).json({ success: false, error: message });
  }
});
